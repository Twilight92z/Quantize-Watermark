...
def data_process(model, mode, train_size, test_size, trigger="enlottoos n tg oto dbmm Iyls eitg"):
    ...
    if mode == "normal":
        ...
        train.extend(make_items(train_normal, train_normal_ids, 0))
        ...
        train_wm = add_wm(train_wm, "wiki")
        test_wm, test_normal = cut_test(test_wm, "wiki"), cut_test(test_normal, "wiki")
    ...
    with open(f"../../data/maintain_int8/{model}_{mode}_train.json", "w") as f:
        for each in train:
            f.write(json.dumps(each) + "\n")      
    with open(f"../../data/maintain_int8/{model}_{mode}_test.json", "w") as f:
        for each in test:
            f.write(json.dumps(each) + "\n")
