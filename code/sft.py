...
class Trainer:
    def __init__(self, model, pretrain_model, devices, save_param_folder, train_loader, epochs, learning_rate=3e-5):
        """
            devices `List`
...
def main():
    ...
    model = args.model
    train_set = load_dataset("json", data_files=f"../data/{model}_sft.json")["train"].with_format("torch")
    train_loader = DataLoader(train_set, batch_size=8, shuffle=True)
    trainer = Trainer(model, args.pretrain_model, devices, args.save_param_path, train_loader, args.epochs, args.learning_rate)
    trainer.train()
